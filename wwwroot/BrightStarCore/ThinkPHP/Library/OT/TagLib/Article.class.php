...
class Article extends TagLib{
    ...
    protected $tags   =  array(
        ...
        'page'     => array('attr' => 'cate,listrow,class', 'close' => 0), //列表分页
        'position' => array('attr' => 'pos,cate,limit,filed,name', 'close' => 1), //获取推荐位列表
        'list'     => array('attr' => 'name,category,child,row,page,field,pos', 'close' => 1), //获取指定分类列表
    );

    public function _list($tag, $content){
        $name   = $tag['name'];
        $cate   = !empty($tag['category']) ? $tag['category'] : $tag['cate'];
        $child  = empty($tag['child']) ? 'false' : $tag['child'];
        $row    = empty($tag['row'])   ? '10' : $tag['row'];
        $page    = !empty($tag['page'])   ? $tag['page'] : (!empty($_GET["p"])?$_GET["p"]:1);
    ...
    public function _page($tag){
        $cate    = !empty($tag['category']) ? $tag['category'] : $tag['cate'];
        $listrow = $tag['listrow'];
        $class   = empty($tag['class']) ? '\Think\Page' : $tag['class'];
        $parse   = '<?php ';
        $parse  .= '$__PAGE__ = new '.$class.'(get_list_count(' . $cate . '), ' . $listrow . ');';
