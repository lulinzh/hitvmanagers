...
class Think extends TagLib{
    ...
    protected $tags   =  array(
        ...
        'nav'       =>  array('attr' => 'field,name,tree', 'close' => 1), //获取导航
        'link'       =>  array('attr' => 'group,name', 'close' => 1), //获取网站连接 add by heqh
    ...
    public function _link($tag, $content){
    	...
    	$parse  = $parse   = '<?php ';
    	if( empty($group) ){
    		$parse .= '$__LINK__ = M(\'Link\')->where("1=1")->order("sort,id")->select();';
    	}else{
    		$parse .= '$__LINK__ = M(\'Link\')->where("`group`=\''.$group.'\'")->order("sort,id")->select();';
    	}
